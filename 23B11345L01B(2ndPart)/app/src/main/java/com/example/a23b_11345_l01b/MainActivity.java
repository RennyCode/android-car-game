...
public class MainActivity extends AppCompatActivity {
    ...
    private final int FUEL2_INDEX = 4;
    private final int MARG_RIGHT = 110;
    ...
    private final int DELAY = 850;
    private final int SHORT_DELAY = 150;
...
//    private ShapeableImageView[] main_IMG_fuels;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println("from menu intent got : lat = " + lat + ", lon = " + lon);
...
//            gameManager.getFuels()[i].set_y_pos(-3*i);
//            gameManager.getFuels()[i].set_x_pos((i+3)%3);
//            main_IMG_fuels[i].setY(gameManager.getFuels()[i].get_y_pos() * step_size_y);
//            main_IMG_fuels[i].setX(gameManager.getFuels()[i].get_x_pos() * step_size_x + MARG_RIGHT);
//        }
    }
    ...
    private void phone_motion(int pos) {
        ...
        if (pos == 2) {
            ...
            gameManager.getUser().set_x_pos(1);
            main_IMG_car.setX(gameManager.getUser().get_x_pos() * step_size_x + MARG_RIGHT);
        }
            ...
            for(int i=0; i<gameManager.getObstacles().length; i++) {
                ...
                if (i_y_pos > 12) {
                    ...
                    if(gameManager.getObstacles()[i].getIsFuel()) {
                        main_IMG_obstacles[i].setRotation(0);
                    }
                    main_IMG_obstacles[i].setY(gameManager.getObstacles()[i].get_y_pos() * step_size_y);
                    main_IMG_obstacles[i].setX(gameManager.getObstacles()[i].get_x_pos() * step_size_x + MARG_RIGHT);
                } else {
...
//            for(int i=0; i<gameManager.getFuels().length; i++) {
//
//                int i_y_pos = gameManager.getFuels()[i].get_y_pos();
//                if (i_y_pos > 12) {
//                    //obs reach to bottom
//                    gameManager.getFuels()[i].reset_pos();
//                    main_IMG_fuels[i].setY(gameManager.getFuels()[i].get_y_pos() * step_size_y);
//                    main_IMG_fuels[i].setX(gameManager.getFuels()[i].get_x_pos() * step_size_x + MARG_RIGHT);
//                } else {
//                    //move obj down
//                    gameManager.getFuels()[i].set_y_pos(i_y_pos + 1);
//                    main_IMG_fuels[i].setY(gameManager.getFuels()[i].get_y_pos() * step_size_y);
//                }
//            }
        }
    ...
    private void openScoreScreen(int score) {
        ...
        System.out.println("from main: score = " + score);
    ...
    private void collisionUI(){
        ...
        for(int i=0; i<gameManager.getObstacles().length; i++) {
            ...
            if (gameManager.getObstacles()[i].get_x_pos() == cur_car_x
                ...
                if (gameManager.getUser().get_got_hit() || main_IMG_obstacles[i].getRotation() == 90) {
                    ...
                    if(gameManager.getObstacles()[i].getIsFuel()){
                        ...
                        if (gameManager.getLife() != 3) {
                            ...
                            main_IMG_obstacles[i].setRotation(90);
                        }
...
//        for(int i=0; i<gameManager.getFuels().length; i++) {
//
//            if (gameManager.getFuels()[i].get_x_pos() == cur_car_x
//                    && cur_car_y - gameManager.getFuels()[i].get_y_pos() < 5
//                    && cur_car_y - gameManager.getFuels()[i].get_y_pos() > 2) {
//
//                    //detect collision
//                    System.out.println("got fuel is true");
//                    gameManager.getUser().set_got_hit(true);
//
//                    if (gameManager.getLife() != 3) {
//                        gameManager.setLife(gameManager.getLife() + 1);
//                        main_IMG_hearts[gameManager.getLife()].setAlpha(1.0f);
//                    }
//            }
//        }
...
//        main_IMG_fuels = new ShapeableImageView[]{
//                findViewById(R.id.main_IMG_fuel_1),
//                findViewById(R.id.main_IMG_fuel_2),
//                findViewById(R.id.main_IMG_fuel_3)};
        ...
        step_size_x = (Resources.getSystem().getDisplayMetrics().widthPixels - 2 * MARG_RIGHT)/LANES;
        step_size_y = (Resources.getSystem().getDisplayMetrics().heightPixels - 2 * MARG_RIGHT)/y_DIVIDER;
    ...
    private void initStepDetector() {
        System.out.println("got in to init");
        stepDetector = new StepDetector(this, new StepCallback() {
            @Override
            public void tilted_to_right() {
                phone_motion(2);
                System.out.println("got in to tilted_to_right");
            }

            @Override
            public void tilted_to_left() {
                phone_motion(0);
                System.out.println("got in to tilted_to_left");
            }

            @Override
            public void tilted_to_center() {
                phone_motion(1);
                System.out.println("got in to tilted_to_centre");
            }
